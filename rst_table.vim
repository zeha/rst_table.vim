...
python << EOF
# -*- encoding: utf-8 -*-
# Note: we do our own from/to unicode, as vim always gives us bytestrings :-(
import vim
import re

def get_current_fileencoding():
    return vim.eval("&fileencoding") or 'ascii'
...
def create_line(columns, widths):
    ...
    Por ejemplo::
        ...
        >>> create_line([u'nombre', u'apellido'], [7, 10])
        u'| nombre  | apellido   |'
    """
    ...
    for text, width in line:
        result.append(u"| " + text.ljust(width) + u" ")
...
def create_table(content):
    ...
    Por ejemplo::
        ...
        >>> print create_table([[u'software', u'vesion'], [u'python', u'2.6'], [u'vim', u'7.2']])
        +----------+--------+
        | software | vesion |
        +==========+========+
        | python   | 2.6    |
    ...
    result.append(create_separarator(widths, u'-'))
...
def extract_cells_as_list(string):
    ...
    return [item.strip() for item in string.split(u'|') if item]

def extract_table(fileencoding, buffer, top, bottom):
    content = []
    full_table_text = [x.decode(fileencoding) for x in buffer[top:bottom]]
    # selecciona solamente las lineas que tienen celdas con texto.
    only_text_lines = [x for x in full_table_text if u'|' in x]
    # extrae las celdas y descarta los separadores innecesarios.
    return [extract_cells_as_list(x) for x in only_text_lines]

def extract_words_as_lists(fileencoding, buffer, top, bottom):
    "Genera una lista de palabras para crear una lista."

    lines = buffer[top:bottom+1]
    return [create_separators_removing_spaces(line.decode(fileencoding)).split(u'|') for line in lines]
...
def fix_table(current_row_index):
    ...
    fileencoding = get_current_fileencoding()
    table_as_list = extract_table(fileencoding, vim.current.buffer, r1, r2)

    # genera una nueva tabla tipo restructured text y la dibuja en el buffer.
    table_content = create_table(table_as_list).encode(fileencoding)
...
def CreateTable():
    ...
    fileencoding = get_current_fileencoding()
    lines = extract_words_as_lists(fileencoding, vim.current.buffer, top, bottom)
    table_content = create_table(lines).encode(fileencoding)
